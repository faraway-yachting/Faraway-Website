    ...
    <div className="bg-white rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 group relative overflow-hidden">
        <div className="flex flex-col items-center text-center gap-4">
            <div className="w-14 h-14 bg-mustard rounded-xl flex items-center justify-center p-3 shadow-md">
                <img src={icon} alt={`${title} icon`} className="w-full h-full object-contain filter brightness-0 invert" />
            </div>
            <div className="space-y-3">
                <h3 className="font-bold text-zink text-xl hover:text-mustard transition-colors duration-300">{title}</h3>
                <p className="text-zink text-base font-sourceSansPro leading-relaxed text-center">{description}</p>
            </div>
        </div>
